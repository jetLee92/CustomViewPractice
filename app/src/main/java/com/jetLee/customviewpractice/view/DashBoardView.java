...
public class DashBoardView extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		double indicatorAngle = (360 - ANGLE) / 20 * scaleValue + 90 + ANGLE / 2;
		indicatorPath.moveTo(getWidth() / 2, getHeight() / 2);
		indicatorPath.rLineTo(((float) Math.cos(Math.toRadians(indicatorAngle))) * INDICATOR_LENGTH,
				((float) Math.sin(Math.toRadians(indicatorAngle))) * INDICATOR_LENGTH);
		canvas.drawPath(indicatorPath, indicatorPaint);
//		canvas.drawLine(getWidth() / 2, getHeight() / 2,
//				((float) Math.cos(Math.toRadians(indicatorAngle))) * INDICATOR_LENGTH + getWidth() / 2,
//				((float) Math.sin(Math.toRadians(indicatorAngle))) * INDICATOR_LENGTH + getHeight() / 2, indicatorPaint);
